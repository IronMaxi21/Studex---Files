...
import { applyBody, chargeStorage } from './filebody.js';
...
export interface PullResult {
  ...
  updated: number;
  /**
   * Both sides had changed. The file kept its identity: the synced version is
   * what it now holds, and the version it replaced is kept as an earlier
...
export async function pullLibrary(
  ...
  for (const item of remote.filter((i) => FILE_KINDS.has(i.kind))) {
    ...
    if (mapped && existing) {
      ...
      if (item.contentHash !== contentBase) {
        if (contentUnchanged) {
          ...
          // Both moved. A file is one file on every device, so this does not
          // fork into a second copy: the version that synced most recently is
          // what the file holds, and the version it replaced is kept behind it
          // as an earlier version this Mac can put back. Nothing is thrown
          // away and nothing is duplicated.
          await snapshotFile(userId, existing, 'sync');
          await applyBody(userId, existing, await store.getObject(item.storagePath!));
          contentBase = item.contentHash;
          clashed = true;
        }
    ...
    if (candidate) {
      ...
      // having agreed — the both-changed case with no base at all. Same answer
      // as above: one file, the incoming version in it, the version it
      // replaced kept behind it.
      await snapshotFile(userId, candidate, 'sync');
      await applyBody(userId, candidate, await store.getObject(item.storagePath!));
      recordState(userId, 'file', candidate.id, item.id, item.contentHash, identity);
      result.conflicted += 1;
      continue;
    }
